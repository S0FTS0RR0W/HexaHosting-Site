      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <Card className="p-6 border-l-4 border-l-green-500">
            <div className="text-sm text-slate-600 dark:text-slate-400">Operational</div>
          ...
          <Card className="p-6 border-l-4 border-l-red-500">
            <div className="text-sm text-slate-600 dark:text-slate-400">Down</div>
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <span className={`inline-block w-3 h-3 rounded-full ${getStatusDot(service.status)}`} />
                        <h3 className="text-lg font-semibold">{service.name}</h3>
                      </div>
                      <p className={`mt-1 text-sm font-medium capitalize ${getStatusColor(service.status)}`}>
                        {service.status}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4">
                    <div>
                      ...
                      <div className={`text-2xl font-bold mt-1 ${getUptimeColor(service.uptime)}`}>
                        {service.uptime}%
                      </div>
                    </div>
                    <div>
        ...
        <div className="mt-12 p-6 bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800 rounded-lg">
          <p className="text-sm text-slate-700 dark:text-slate-300">
            ...
            Data refreshes automatically • For detailed history, check our 
